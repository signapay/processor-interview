...
/**
 * Runs the inital script to bootstrap the DB. I think the script in the docker/sql is run upon docker compose,
 * but wanted to make sure the db tables are present
 */
class BootstrapModel extends AbstractModel{
    ...
    async createTables(sql) {
        let client = null;
        try {
            client = await this._getPool().connect();
            const res = await client.query(this.getSql());
            return true;
        } catch (err) {
