...
export function FileUpload({ onDataReceived }: FileUploadProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Make sure to include all required properties in the error response
      const errorResponse: ApiResponse = {
        success: false,
        message: err instanceof Error ? err.message : "Failed to process file",
        timestamp: new Date().toISOString(), // Add timestamp to ensure type consistency
      }

      onDataReceived(errorResponse)
    } finally {
      setIsLoading(false)
      e.target.value = ""
    }
